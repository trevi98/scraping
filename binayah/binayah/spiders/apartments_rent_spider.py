import scrapy
from ..items import apartments_rentItem
import requests
...
class testingSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.binayah.com/apartments-for-rent-in-dubai-uae/"]
    page_number = 2
    link = ""
    ...
    def parse(self,response):
        ...
        next_page = f"https://www.binayah.com/apartments-for-rent-in-dubai-uae/page/{self.page_number}/"
        if next_page is not None and self.page_number < 8:
            self.page_number +=1
            yield response.follow(next_page,callback = self.parse)
        else:
            # pass
            data = {'message': 'binaya apartments_rent done'}
            response = requests.post("https://notifier.abdullatif-treifi.com/", data=data)
            # sys.path.append('/c/Python310/Scripts/scrapy')

    def page(self,response):
        items = apartments_rentItem()
        ...
        # bedrooms=response.css("div#property-overview-wrap div.property-overview-data ul")[1].css("li strong::text").get()
        # bathrooms=response.css("div#property-overview-wrap div.property-overview-data ul")[2].css("li strong::text").get()
