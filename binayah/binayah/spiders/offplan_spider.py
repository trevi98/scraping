...
class testingSpider(scrapy.Spider):
    ...
    def page(self,response):
        ...
        # try:
        #     description=BeautifulSoup(response.css(".wpb_text_column.wpb_content_element .wpb_wrapper").extract()[2],'lxml').text.replace("\n","").replace("  ","")
        # except:
        #     try:
        ...
        plans_location=response.css("div.wpb_column.vc_column_container").extract()
        for i in range(len(plans_location)):
            if plans_location[i].__contains__("figure"):
                s=i+1
                while s<len(plans_location):
                    if (plans_location[s].__contains__("Location Map")) and ((plans_location[s].__contains__("h2"))):
                        image_location=methods.img_downloader_method_src(plans_location[s-1],signature)
                    elif(plans_location[s].__contains__("Plan")) and ((plans_location[s].__contains__("h2"))):
                        images_floor_plan=methods.img_downloader_method_src(plans_location[s-1],signature)
                    else:
                        break
            else:
                continue
