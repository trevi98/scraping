import scrapy
from ..items import staff_accommodation_saleItem
import requests
...
class testingSpider(scrapy.Spider):
    name = 'staff_accommodation_sale'
    start_urls = ["https://www.binayah.com/staff-accommodation-for-sale-in-dubai-uae/"]
    page_number = 2
    link = ""
    ...
    def parse(self,response):
        ...
        next_page = f"https://www.binayah.com/staff-accommodation-for-sale-in-dubai-uae/{self.page_number}/"
        if next_page is not None and self.page_number < 1:
            self.page_number +=1
            yield response.follow(next_page,callback = self.parse)
        else:
            # pass
            data = {'message': 'binaya staff_accommodation_sale done'}
            response = requests.post("https://notifier.abdullatif-treifi.com/", data=data)
            # sys.path.append('/c/Python310/Scripts/scrapy')

    def page(self,response):
        items = staff_accommodation_saleItem()
