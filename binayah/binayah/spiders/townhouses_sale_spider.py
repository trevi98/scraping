import scrapy
from ..items import townhouses_saleItem
import requests
...
class testingSpider(scrapy.Spider):
    name = 'townhouses_sale'
    start_urls = ["https://www.binayah.com/townhouses-for-sale-in-dubai/"]
    page_number = 2
    link = ""
    ...
    def parse(self,response):
        ...
        next_page = f"https://www.binayah.com/townhouses-for-sale-in-dubai/page/{self.page_number}/"
        if next_page is not None and self.page_number < 7:
            self.page_number +=1
            yield response.follow(next_page,callback = self.parse)
        else:
            # pass
            data = {'message': 'binaya townhouses_sale done'}
            response = requests.post("https://notifier.abdullatif-treifi.com/", data=data)
            # sys.path.append('/c/Python310/Scripts/scrapy')

    def page(self,response):
        items = townhouses_saleItem()
