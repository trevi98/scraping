import scrapy
from ..items import Warehouse_saleItem
import requests
...
class testingSpider(scrapy.Spider):
    name = 'Warehouse_sale'
    start_urls = ["https://www.binayah.com/warehouse-for-sale-in-dubai-uae/"]
    page_number = 2
    link = ""
    ...
    def parse(self,response):
        ...
        next_page = f"https://www.binayah.com/warehouse-for-sale-in-dubai-uae/{self.page_number}/"
        if next_page is not None and self.page_number < 1:
            self.page_number +=1
            yield response.follow(next_page,callback = self.parse)
        else:
            # pass
            data = {'message': 'binaya all done'}
            response = requests.post("https://notifier.abdullatif-treifi.com/", data=data)
            # sys.path.append('/c/Python310/Scripts/scrapy')

    def page(self,response):
        items = Warehouse_saleItem()
