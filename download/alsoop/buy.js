...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/buy_alsop${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("buy_alsop");
let csvWriter = csv_handler("buy_alsop", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("buy_alsop", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.allsoppandallsopp.com/dubai/properties/residential/sales/page-${i}`;
  if (i == 1) {
    target =
      "https://www.allsoppandallsopp.com/dubai/properties/residential/sales";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 106) {
    const message = `Done - buy_alsop_alsop driv ${i} done`;
