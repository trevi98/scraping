...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/rent_alsop${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("rent_alsop");
let csvWriter = csv_handler("rent_alsop", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("rent_alsop", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.allsoppandallsopp.com/dubai/properties/residential/lettings/page-${i}`;
  if (i == 1) {
    target =
      "https://www.allsoppandallsopp.com/dubai/properties/residential/lettings";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 31) {
    const message = `Done - rent_alsop_alsop  ${i} done`;
