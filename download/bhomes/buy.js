...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/buy_bhomes${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("buy_bhomes");
let csvWriter = csv_handler("buy_bhomes", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("buy_bhomes", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.bhomes.com/en/buy?locations=uae_dubai&page=${i}`;
  if (i == 1) {
    target = "https://www.bhomes.com/en/buy?locations=uae_dubai";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 208) {
    const message = `Done - buy bhomes ${i} done`;
