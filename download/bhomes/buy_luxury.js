...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/buy_luxury_bhome${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("buy_luxury_bhome");
let csvWriter = csv_handler("buy_luxury_bhome", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("buy_luxury_bhome", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 10 == 0 || i == 13) {
    const message = `Done - buy_luxury_bhome  ${i} done`;
