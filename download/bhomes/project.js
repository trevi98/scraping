...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/project_bhome${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("project_bhome");
let csvWriter = csv_handler("project_bhome", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("project_bhome", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 10 == 0 || i == 7) {
    const message = `Done - project_bhome  ${i} done`;
