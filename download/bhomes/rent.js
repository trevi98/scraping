...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/rent_bhomes${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("rent_bhomes");
let csvWriter = csv_handler("rent_bhomes", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("rent_bhomes", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 20 == 0 || i == 83) {
    const message = `Done - rent bhomes ${i} done`;
