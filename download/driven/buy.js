...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/buy_deiven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("buy_deiven");
let csvWriter = csv_handler("buy_deiven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("buy_deiven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/properties-for-sale?page=${i}`;
  if (i == 1) {
    target = "https://www.drivenproperties.com/dubai/properties-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 133) {
    const message = `Done - buy driv ${i} done`;
