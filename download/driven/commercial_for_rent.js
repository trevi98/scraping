...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/commercail_rent_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("commercail_rent_driven");
let csvWriter = csv_handler("commercail_rent_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("commercail_rent_driven", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 20 == 0 || i == 34) {
    ...
    if (i == 34) {
      exec("pm2 stop main_driv_rent_comm", (error, stdout, stderr) => {
        if (error) {
