...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/commercail_buy_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("commercail_buy_driven");
let csvWriter = csv_handler("commercail_buy_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("commercail_buy_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/commercial-properties-for-sale?page=${i}`;
  if (i == 1) {
    target =
      "https://www.drivenproperties.com/dubai/commercial-properties-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 1) {
    const message = `Done - buy comm driv ${i} done`;
    ...
    if (i === 1) {
      exec("pm2 stop main_driv_buy_comm", (error, stdout, stderr) => {
        if (error) {
