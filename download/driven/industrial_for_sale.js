...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/industrial_for_sale_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("industrial_for_sale_driven");
let csvWriter = csv_handler("industrial_for_sale_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("industrial_for_sale_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/industrial-warehouses-for-sale?page=${i}`;
  if (i == 1) {
    target =
      "https://www.drivenproperties.com/dubai/industrial-warehouses-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 4) {
    const message = `Done - indus_sale driv ${i} done`;
    ...
    if (i == 4) {
      exec("pm2 stop main_indus", (error, stdout, stderr) => {
        if (error) {
