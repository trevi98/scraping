...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/luxury_villas_for_rent_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("luxury_villas_for_rent_driven");
let csvWriter = csv_handler("luxury_villas_for_rent_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("luxury_villas_for_rent_driven", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 20 == 0 || i == 1) {
    const message = `Done - luxury_villas_for_rent_driven driv ${i} done`;
