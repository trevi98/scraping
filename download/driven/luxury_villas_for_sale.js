...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/luxury_villas_for_sale_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("luxury_villas_for_sale_driven");
let csvWriter = csv_handler("luxury_villas_for_sale_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("luxury_villas_for_sale_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/luxury-villas-for-sale?page=${i}`;
  if (i == 1) {
    target = "https://www.drivenproperties.com/dubai/luxury-villas-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 2) {
    const message = `Done - luxury_villas_for_sale_driven driv ${i} done`;
