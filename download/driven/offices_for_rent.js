...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offices_for_rent_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offices_for_rent_driven");
let csvWriter = csv_handler("offices_for_rent_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offices_for_rent_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/properties-for-rent/office?page=${i}`;
  if (i == 1) {
    target =
      "https://www.drivenproperties.com/dubai/properties-for-rent/office";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 25) {
    const message = `Done - office_rent driv ${i} done`;
    ...
    if (i == 25) {
      exec("pm2 stop main_office", (error, stdout, stderr) => {
        if (error) {
