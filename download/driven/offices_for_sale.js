...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/office_buy_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("office_buy_driven");
let csvWriter = csv_handler("office_buy_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("office_buy_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/offices-for-sale?page=${i}`;
  if (i == 1) {
    target = "https://www.drivenproperties.com/dubai/offices-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 4) {
    const message = `Done - office_buy driv ${i} done`;
