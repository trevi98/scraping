...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offplan_properties_apartments_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offplan_properties_apartments_driven");
let csvWriter = csv_handler("offplan_properties_apartments_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  data.push(
    await page.evaluate(async () => {
      ...
      if (temp.length == 0)
        temp = Array.from(
          document.querySelectorAll(
            ".dpx-project-privileged-location-area ul li"
          )
        );
      temp.forEach((e) => {
        try {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offplan_properties_apartments_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/off-plan-apartments-for-sale?page=${i}`;
  if (i == 1) {
    target =
      "https://www.drivenproperties.com/dubai/off-plan-apartments-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 2) {
    const message = `Done - offplan_properties_apartments_driven driv ${i} done`;
