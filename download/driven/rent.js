...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/rent_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("rent_driven");
let csvWriter = csv_handler("rent_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("rent_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/properties-for-rent?page=${i}`;
  if (i == 1) {
    target = "https://www.drivenproperties.com/dubai/properties-for-rent";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 109) {
    const message = `Done - rent driv ${i} done`;
    ...
    if (i == 109) {
      exec("pm2 stop main_driv_rent", (error, stdout, stderr) => {
        if (error) {
