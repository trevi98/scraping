...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/shops_for_sale_driven${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("shops_for_sale_driven");
let csvWriter = csv_handler("shops_for_sale_driven", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("shops_for_sale_driven", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.drivenproperties.com/dubai/retail-shops-for-sale?page=${i}`;
  if (i == 1) {
    target = "https://www.drivenproperties.com/dubai/retail-shops-for-sale";
  }
  console.log(target);
  ...
  if (i == 1 || i % 20 == 0 || i == 1) {
    const message = `Done - shops_for_sale driv ${i} done`;
    ...
    if (i == 1) {
      exec("pm2 stop main_shop", (error, stdout, stderr) => {
        if (error) {
