...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/commercail_buy${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("commercail_buy");
let csvWriter = csv_handler("commercail_buy", 1);
let batch = 0;
let j = 0;
...
async function visit_each(obj, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("commercail_buy", batch);
  }
...
async function main_loop(page) {
  let target = `https://famproperties.com/commercial-properties-for-sale-in-dubai`;
  ...
  for (const obj of all_) {
    ...
    if (
      ...
      const message = `Data -  Fam commercail_buy ${
        all_.indexOf(obj) + 1
      } done`;
