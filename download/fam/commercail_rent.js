...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/commercail_rent${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("commercail_rent");
let csvWriter = csv_handler("commercail_rent", 1);
let batch = 0;
let j = 0;
...
async function visit_each(obj, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("commercail_rent", batch);
  }
...
async function main_loop(page) {
  let target = `https://famproperties.com/commercial-properties-for-rent-in-dubai`;
  ...
  for (const obj of all_) {
    ...
    if (
      ...
      const message = `Data -  Fam commercail_rent ${
        all_.indexOf(obj) + 1
      } done`;
