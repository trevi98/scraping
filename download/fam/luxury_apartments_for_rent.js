...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/luxury_apartments_for_rent${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("luxury_apartments_for_rent");
let csvWriter = csv_handler("luxury_apartments_for_rent", 1);
let batch = 0;
let j = 0;
...
async function visit_each(obj, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("luxury_apartments_for_rent", batch);
  }
...
async function main_loop(page) {
  let target = `https://famproperties.com/luxury-apartments-for-rent-dubai`;
  ...
  for (const obj of all_) {
    ...
    if (
      ...
      const message = `Data -  Fam luxury_apartments_for_rent ${
        all_.indexOf(obj) + 1
      } done`;
