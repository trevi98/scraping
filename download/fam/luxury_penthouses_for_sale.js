...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/luxury_penthouses_for_sale${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("luxury_penthouses_for_sale");
let csvWriter = csv_handler("luxury_penthouses_for_sale", 1);
let batch = 0;
let j = 0;
...
async function visit_each(obj, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("luxury_penthouses_for_sale", batch);
  }
...
async function main_loop(page) {
  let target = `https://famproperties.com/luxury-penthouses-for-sale-dubai`;
  ...
  for (const obj of all_) {
    ...
    if (
      ...
      const message = `Data -  Fam luxury_penthouses_for_sale ${
        all_.indexOf(obj) + 1
      } done`;
