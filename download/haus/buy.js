...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/buy_haus${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("buy_haus");
let csvWriter = csv_handler("buy_haus", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("buy_haus", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 20 == 0 || i == 64) {
    const message = `Done - buy haus ${i} done`;
