...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/tenants_guide_spider_haus${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("area_guitenants_guide_spider_hausdes_haus");
let csvWriter = csv_handler("tenants_guide_spider_haus", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("tenants_guide_spider_haus", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.hausandhaus.com/property-leasing/tenants-guide`;
  console.log(target);
