...
let csvErrr = csv_error_handler("project_metro");
let csvWriter = csv_handler("project_metro", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  data.push(
    await page.evaluate(async () => {
      ...
      let title = clean(
        document
          .querySelector(".projectHeading  h2")
          .textContent.replace("About", "")
      );

      let price =
        extract_one_text_from_pare_elements_in_one_container__pass_array_of_main_containers(
          Array.from(
            document.querySelectorAll(
              ".container .row.advRow.as_grid .as_grid-item"
            )
          ),
          "Starting Price from",
          "span.as_grid-params"
      ...
      let area =
        extract_one_text_from_pare_elements_in_one_container__pass_array_of_main_containers(
          Array.from(document.querySelectorAll(".container .as_lits li")),
          "Location",
          "span.as_lits-params"
        );
      let status =
      ...
      try {
        entire_description = clean(
          Array.from(
            document.querySelectorAll(".contentSection.featureProjects")
          )[1].innerText
        );
      } catch (error) {
        entire_description = "";
      ...
      amenities = Array.from(
        document.querySelectorAll(
          ".container .two-col-text .two-col-text-item.text-text ul li"
        ),
        (li) => li.textContent
      );
      if (amenities.length == 0) {
        amenities = Array.from(
          document.querySelectorAll(".container .two-col-text ul li"),
          (li) => li.textContent
        );
      }
      if (amenities.length == 0) {
        amenities = Array.from(
          document.querySelectorAll(
            ".container .two-col-text .two-col-text-item.grid-text ul li"
          ),
          (li) => li.textContent
        );
      }
      if (amenities.length == 0) {
        amenities = Array.from(
          document.querySelectorAll(
            ".container .two-col-text .two-col-text-item.video-text ul li"
          ),
          (li) => li.textContent
        );
      }
      ...
      let temp = Array.from(
        document.querySelectorAll(
          ".container.fp .fp__slider.fp-slider .fpSlider .owl-item:not(.cloned)"
        )
      );
      temp.forEach((t) => {
        let items_container = t.querySelector(
          ".fpSlider__col.fpSlider__col--desc"
        );
        let title = clean(
          items_container.querySelector(".fpSlider__desc-head").textContent
        );
        let pares = Array.from(
          items_container.querySelectorAll(".fpSlider__desc-item")
        );
        let type =
          extract_one_text_from_pare_elements_in_one_container__pass_array_of_main_containers(
            pares,
        ...
        let image = t
          .querySelector(".fpSlider__col.fpSlider__col--image img")
          .getAttribute("data-src");
        // console.log("!!!!")
        // console.log(image)

        floor_plans.push(
          JSON.stringify({ title, type, total_area, starting_price, image })
        );
      });

      let images = Array.from(
        document.querySelectorAll(".gallerySlider__item img"),
        (img) => img.getAttribute("data-src")
      );
      temp = Array.from(
        document.querySelectorAll(
          ".container .row.advRow.as_grid .as_grid-cell.col-1"
        )
      );
      payment_plan =
        create_pares_from_pare_elements_in_one_container_if_thing_exists__pass_array_of_pares_containers(
          temp,
          "%",
          ".as_grid-title",
          ".as_grid-params"
      ...
      x = extract_text_from_pare_elements__section__(
        temp,
        "Location",
        ".container .two-col-text"
      );
      economic_apeal = extract_text_from_pare_elements__section__(
        temp,
        "Economic Appeal",
        ".container .textBlock.pd30.full"
      );
      property_price = extract_text_from_pare_elements__section__(
        temp,
        "Property Prices",
        ".container .textBlock.pd30.full"
      );
  ...
  page.once("request", (request) => {
    if (request.url().includes("-fp.pdf")) {
      data[0].floor_plans_pdf = request.url();
      request.abort();
    } else if (request.url().includes("pdf")) {
      data[0].brochure = request.url();
      request.abort();
    } else {
      request.continue();
    }
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("project_metro", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 20 == 0) {
    const message = `Done - project metro ${i} done`;
    ...
    if (i == 180) {
      exec("pm2 stop main_metro", (error, stdout, stderr) => {
        if (error) {
