...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/opr_Buy_Beachfront_${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("opr_Buy_Beachfront_");
let csvWriter = csv_handler("opr_Buy_Beachfront_", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("opr_Buy_Beachfront_", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i > 0) {
    const message = `Data - Opr  Buy_Beachfront ${i} done`;
...
async function main() {
  const filePath = "opr_Buy_Beachfront_stop.txt";
