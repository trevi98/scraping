...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/opr_Buy_Luxury${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("opr_Buy_Luxury");
let csvWriter = csv_handler("opr_Buy_Luxury", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("opr_Buy_Luxury", batch);
  }
...
async function main_loop(page, i) {
  ...
  exec("pm2 stop main_luxury", (error, stdout, stderr) => {
    if (error) {
  ...
  if (i > 0) {
    const message = `Data - Opr buy near ${i} done`;
...
async function main() {
  const filePath = "opr_buy_luxury_stop.txt";
