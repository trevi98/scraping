...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/opr_Buy_Near_Golf_Course22${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("opr_Buy_Near_Golf_Course22");
let csvWriter = csv_handler("opr_Buy_Near_Golf_Course22", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("opr_Buy_Near_Golf_Course22", batch);
  }
  ...
  for (const link of links) {
    ...
    if (
      ...
      if (links.indexOf(link) === links.length - 1) {
        exec("pm2 stop main_near", (error, stdout, stderr) => {
          if (error) {
...
// if (i > 0) {
//   const message = `Data - Opr buy_near projects ${i} done`;
...
async function main() {
  const filePath = "opr_water_front_stop.txt";
