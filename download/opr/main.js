...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/opr_Buy_Near_Golf_Course${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("opr_Buy_Near_Golf_Course");
let csvWriter = csv_handler("opr_Buy_Near_Golf_Course", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("opr_Buy_Near_Golf_Course", batch);
  }
...
async function main() {
  const filePath = "opr_buy_near_stop.txt";
