...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/communities_prov${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("communities_prov");
let csvWriter = csv_handler("communities_prov", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("communities_prov", batch);
  }
...
async function main_loop(page, i) {
  ...
  if (i == 1 || i % 20 == 0 || i == 6) {
    const message = `Done - comm prov ${i} done`;
