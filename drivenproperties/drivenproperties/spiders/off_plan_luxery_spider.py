...
class testingSpider(scrapy.Spider):
    name = 'offplan_luxery'
    start_urls = ["https://www.drivenproperties.com/dubai/properties-for-sale/off-plan/exclusive-projects"]
    page_number = 2
    link = ""
    ...
    def parse(self,response):
        ...
        next_page = f"https://www.drivenproperties.com/dubai/properties-for-sale/off-plan/exclusive-projects?page={self.page_number}/"
        if next_page is not None and self.page_number < 1:
            self.page_number +=1
            yield response.follow(next_page,callback = self.parse)
        else:
            # pass
            data = {'message': 'driven offplan luxery done'}
