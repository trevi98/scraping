...
if(btns.length>0){
  ...
  data.push(await page.evaluate(async ()=>{
    ...
    let area = Array.from(document.querySelectorAll("span._327a3afc"),elmnt=>elmnt.textContent)[2].replace('\n','').replace('\r','').replace('\t','').replace('  ','')
    let price = document.querySelector("span._105b8a67").textContent.replace('\n','').replace('\r','').replace('\t','').replace('  ','')
    ...
    let description = document.querySelector("._2015cd68.d320ecf0").textContent.replace('\n','').replace('\r','').replace('\t','').replace('  ','')
    let amenities = Array.from(document.querySelectorAll("div._4b64e3bd div._9676c577"),elmnt=>elmnt.textContent.replace('\n','').replace('\r','').replace('\t','').replace('  ',''))
