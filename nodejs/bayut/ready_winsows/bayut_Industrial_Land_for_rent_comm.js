...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/bayut_Industrial_Land_for_rent_comm${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("bayut_Industrial_Land_for_rent_comm");
let csvWriter = csv_handler("bayut_Industrial_Land_for_rent_comm", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("bayut_Industrial_Land_for_rent_comm", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.bayut.com/to-rent/industrial-land/uae/page-${i}/?rent_frequency=any`;
  if (i == 1) {
    target =
      "https://www.bayut.com/to-rent/industrial-land/uae/?rent_frequency=any";
  }
  console.log(target);
