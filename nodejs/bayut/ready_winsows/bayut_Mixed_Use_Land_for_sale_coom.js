...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/bayut_Mixed_Use_Land_for_sale_coom${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("bayut_Mixed_Use_Land_for_sale_coom");
let csvWriter = csv_handler("bayut_Mixed_Use_Land_for_sale_coom", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("bayut_Mixed_Use_Land_for_sale_coom", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.bayut.com/for-sale/mixed-use-land/uae/page-${i}/`;
  if (i == 1) {
    target = "https://www.bayut.com/for-sale/mixed-use-land/uae/";
  }
  console.log(target);
