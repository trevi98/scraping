...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/bayut_Properties_for_sale_res${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("bayut_Properties_for_sale_res");
let csvWriter = csv_handler("bayut_Properties_for_sale_res", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("bayut_Properties_for_sale_res", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.bayut.com/for-sale/property/uae/page-${i}/`;
  if (i == 1) {
    target = "https://www.bayut.com/for-sale/property/uae/";
  }
  console.log(target);
