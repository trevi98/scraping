...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/bayut_Shops_for_sale_comm${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("bayut_Shops_for_sale_comm");
let csvWriter = csv_handler("bayut_Shops_for_sale_comm", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("bayut_Shops_for_sale_comm", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.bayut.com/for-sale/shops/uae/page-${i}/`;
  if (i == 1) {
    target = "https://www.bayut.com/for-sale/shops/uae/";
  }
  console.log(target);
