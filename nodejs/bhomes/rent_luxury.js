...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/rent_luxury${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("rent_luxury");
let csvWriter = csv_handler("rent_luxury", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("rent_luxury", batch);
  }
