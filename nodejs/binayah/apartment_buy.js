...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/apartment_buy${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("apartment_buy");
let csvWriter = csv_handler("apartment_buy", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("apartment_buy", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.binayah.com/apartments-for-sale-in-dubai-uae/page/${i}/`;
  if (i == 1) {
    target = "https://www.binayah.com/apartments-for-sale-in-dubai-uae";
  }
  console.log(target);
