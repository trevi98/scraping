...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/buy_luxury_residential${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("buy_luxury_residential");
let csvWriter = csv_handler("buy_luxury_residential", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("buy_luxury_residential", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://dandbdubai.com/luxury-property/residential/dubai/properties-for-sale`;
  console.log(target);
