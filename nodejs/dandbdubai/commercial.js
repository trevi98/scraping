...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/commercial${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("commercial");
let csvWriter = csv_handler("commercial", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("commercial", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://dandbdubai.com/commercial`;
  console.log(target);
