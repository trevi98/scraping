...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/damac_360_vitrual_tours_emmar${batch}.csv`,
...
let csvErrr = csv_error_handler("damac_360_vitrual_tours_emmar");
let csvWriter = csv_handler("damac_360_vitrual_tours_emmar", 1);
let batch = 0;
let j = 0;
...
async function visit_each(all, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("damac_360_vitrual_tours_emmar", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://offplandxb.ae/emaar-360-virtual-tours/`;
