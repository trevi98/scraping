...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/Buy_Green_Areas${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("Buy_Green_Areas");
let csvWriter = csv_handler("Buy_Green_Areas", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("Buy_Green_Areas", batch);
  }
