...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/opr_green_area${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("opr_green_area");
let csvWriter = csv_handler("opr_green_area", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  data.push({
    ...
    price: all.price,
    info: all.info,
    handover: all.handover,
    overview_title: all.overview_title,
    overview: all.overview,
    about: all.about,
    proprty_info: all.proprty_info,
    location: all.location,
    nearby_place: all.nearby_place,
    payment_plan: all.payment_plan,
    signaturea: all.signaturea,
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("opr_green_area", batch);
  }
...
async function main_loop(page, i) {
  let target = "https://opr.ae/projects/green-properties-in-dubai";
  ...
  //   exec("pm2 stop main_green", (error, stdout, stderr) => {
  //     if (error) {
  ...
  //   if (i > 0) {
  //     const message = `Data - Opr buy green ${i} done`;
...
async function main() {
  //   const filePath = "opr_buy_green_stop.txt";
