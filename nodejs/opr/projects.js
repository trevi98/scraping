...
//   await page.waitForNavigation()
  ...
  const links = await page.evaluate(() => {
      const items = Array.from(document.querySelectorAll('.node.section-clear.section.css88 .code #OffPlanListing #addListing .offPlanListing  .offPlanListing__item'));
      let anchors = []
      items.forEach(item => {
        let a = item.querySelector('.offPlanListing__item-titleLink').href
        let types = Array.from(item.querySelectorAll('.offPlanListing__item-type'),type=>type.textContent)
    ...
    try{
      ...
      try{
        ...
        try{
          ...
          console.error(error)
          csvErrr.writeRecords({link:link.link,error:err})
