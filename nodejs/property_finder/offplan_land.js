...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offplan_land${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offplan_land");
let csvWriter = csv_handler("offplan_land", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offplan_land", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.propertyfinder.ae/en/new-projects?page=${i}&property_type[]=9&sort=-featured`;
