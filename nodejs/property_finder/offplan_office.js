...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offplan_office${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offplan_office");
let csvWriter = csv_handler("offplan_office", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offplan_office", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.propertyfinder.ae/en/new-projects?page=${i}&property_type[]=4&sort=-featured`;
