...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offplan_Meraas${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offplan_Meraas");
let csvWriter = csv_handler("offplan_Meraas", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offplan_Meraas", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.providentestate.com/dubai-offplan/dubai-developer/meraas/page/${i}`;
  if (i == 1) {
    target =
      "https://www.providentestate.com/dubai-offplan/dubai-developer/meraas";
  }
  console.log(target);
