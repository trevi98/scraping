...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offplan_Nakheel${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offplan_Nakheel");
let csvWriter = csv_handler("offplan_Nakheel", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offplan_Nakheel", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.providentestate.com/dubai-offplan/dubai-developer/nakheel/page/${i}`;
  if (i == 1) {
    target =
      "https://www.providentestate.com/dubai-offplan/dubai-developer/nakheel";
  }
  console.log(target);
