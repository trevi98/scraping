...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/offplan_Sobha_Group${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("offplan_Sobha_Group");
let csvWriter = csv_handler("offplan_Sobha_Group", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("offplan_Sobha_Group", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.providentestate.com/dubai-offplan/dubai-developer/sobha-group/page/${i}`;
  if (i == 1) {
    target =
      "https://www.providentestate.com/dubai-offplan/dubai-developer/sobha-group";
  }
  console.log(target);
