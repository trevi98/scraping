...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/rent_Commercial_Properties${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("rent_Commercial_Properties");
let csvWriter = csv_handler("rent_Commercial_Properties", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("rent_Commercial_Properties", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.providentestate.com/dubai/commercial-properties-for-rent.html?paged=${i}`;
  if (i == 1) {
    target =
      "https://www.providentestate.com/dubai/commercial-properties-for-rent.html";
  }
  console.log(target);
