...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/sale_Townhouse${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("sale_Townhouse");
let csvWriter = csv_handler("sale_Townhouse", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("sale_Townhouse", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.providentestate.com/dubai/townhouse-for-sale.html?paged=${i}`;
  if (i == 1) {
    target = "https://www.providentestate.com/dubai/townhouse-for-sale.html";
  }
  console.log(target);
