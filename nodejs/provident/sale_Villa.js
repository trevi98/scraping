...
function csv_handler(directory, batch) {
  ...
  return createCsvWriter({
    path: `${directory}/sale_Villa${batch}.csv`,
    header: [
...
let csvErrr = csv_error_handler("sale_Villa");
let csvWriter = csv_handler("sale_Villa", 1);
let batch = 0;
let j = 0;
...
async function visit_each(link, page) {
  ...
  if (j % 500 == 0) {
    batch++;
    csvWriter = csv_handler("sale_Villa", batch);
  }
...
async function main_loop(page, i) {
  let target = `https://www.providentestate.com/dubai/villa-for-sale.html?paged=${i}`;
  if (i == 1) {
    target = "https://www.providentestate.com/dubai/villa-for-sale.html";
  }
  console.log(target);
