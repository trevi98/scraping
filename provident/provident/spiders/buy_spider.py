...
class HausspiderSpider(scrapy.Spider):
    ...
    def page(self,response):
        ...
        property_details=[]
        property_key=response.css("div.iwp-single-property-detail div.iwp-property-block-content div.row div.col-sm-6.col-xs-12.col-lg-6.col-md-6 div.iwp-items div.iwp-item span::text").extract()
        property_value=response.css("div.iwp-single-property-detail div.iwp-property-block-content div.row div.col-sm-6.col-xs-12.col-lg-6.col-md-6 div.iwp-items div.iwp-item::text").extract()
        for i in range(len(property_key)):
            property_details.append({property_key[i]:property_value[i].replace("\n","").replace("  ","")})
